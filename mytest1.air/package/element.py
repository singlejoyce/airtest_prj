...
class Element(object):
    ...
    def __uidump(self):
        ...
        self.utils.cmd("adb shell uiautomator dump --compressed /data/local/tmp/uidump.xml")
        # else:
        #     self.utils.cmd("adb shell uiautomator dump /data/local/tmp/uidump.xml").wait()
        self.utils.cmd("adb pull data/local/tmp/uidump.xml %s" % self.tempFile)
        self.utils.cmd("adb shell rm /data/local/tmp/uidump.xml")
    ...
    def getElementBoundByPackage(self, packageName):
        """
        通过package获取单个元素的区域
        """
        return self.__bound("package", packageName)
